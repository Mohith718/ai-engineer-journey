...
### Project 4: AI Support Ticket Triage Agent
`LangGraph` `FastAPI` `ChromaDB` `Gemini Embeddings` `Groq LLaMA` `Docker` `HTML/CSS/JS`

**Live demo:** [ai-engineer-journey-production-4b12.up.railway.app](https://ai-engineer-journey-production-4b12.up.railway.app)

Reads a support ticket, classifies it, searches a knowledge base for a relevant answer, and either resolves it automatically or escalates it to the right human team, with an honest internal note explaining why. Built around one deliberate constraint: the agent has to be able to say "I don't know" instead of guessing.

Four-node LangGraph pipeline (classify, retrieve, resolve, route output), a hand-built HTML/CSS/JS frontend calling the API directly with `fetch()`, containerized with Docker, deployed live on Railway. Includes a documented real incident: a leaked API key caught by Google's automated scanner, diagnosed from raw deployment logs, and fixed by rotating the key and correcting the git history, not by guessing.
...
| 8-9 | FastAPI, Docker, live deployment — RAG chatbot and Research Agent shipped |
| 9-10 | Second full agent build: multi-node ticket triage system, hand-built frontend, deployed |
...
**Automation:** n8n, VAPI
**Other:** Python, Git, Docker, tiktoken, python-dotenv
